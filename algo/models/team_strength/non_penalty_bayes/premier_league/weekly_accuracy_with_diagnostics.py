...
def predict_gw(df, actual_df, gw, gw_start, gw_end):
    ...
    kish_neff = (weight_sum ** 2) / (weights ** 2).sum()
    ...
    for team, idx in team_map.items():
        prior_att_mu, prior_att_sigma = MANUAL_ATT_PRIORS[team]
        prior_def_mu, prior_def_sigma = MANUAL_DEF_PRIORS[team]

        posterior_rows.append({
            "gw": gw,
            "team": team,
            "prior_att": prior_att_mu,
            "prior_att_sigma": prior_att_sigma,
            "posterior_att": float(att[:, idx].mean()),
            "posterior_att_sd": float(att[:, idx].std()),
            "att_shift": float(att[:, idx].mean() - prior_att_mu),
            "att_shift_sigma": float((att[:, idx].mean() - prior_att_mu) / prior_att_sigma),
            "prior_def": prior_def_mu,
            "prior_def_sigma": prior_def_sigma,
            "posterior_def": float(defn[:, idx].mean()),
            "posterior_def_sd": float(defn[:, idx].std()),
            "def_shift": float(defn[:, idx].mean() - prior_def_mu),
            "def_shift_sigma": float((defn[:, idx].mean() - prior_def_mu) / prior_def_sigma),
        })
    ...
    return {
        ...
        'ess': {
            ...
            "matches": train_df["match_id"].nunique(),
            "weight_sum": float(weight_sum),
            "kish_neff": float(kish_neff),
            "rows_per_match": len(train_df) / train_df["match_id"].nunique(),
            "neff_per_match": float(kish_neff / train_df["match_id"].nunique()),
        },
...
def main():
    ...
    ess_df = pd.DataFrame([r["ess"] for r in records])
    posterior_df = pd.DataFrame(
        [row for r in records for row in r["posterior_rows"]]
    )
    ...
    ess_out = os.path.join(out_dir, "effective_sample_size.csv")
    ess_df.to_csv(ess_out, index=False)
    print(f"Saved: {ess_out}")

    posterior_out = os.path.join(out_dir, "posterior_drift.csv")
    posterior_df.to_csv(posterior_out, index=False)
    print(f"Saved: {posterior_out}")
