"""
Derives Premier League team-season totals (non-penalty xG, non-penalty xGA,
non-penalty goals, non-penalty goals against, possession) from the local
fotmob.db, for the same schema as champ_xg_history.csv. Non-penalty because
this feeds the non_penalty_bayes team strength model - matches m.home_goals/
away_goals would include penalties.

Sources:
  - np_matches for goals (already penalty-stripped by fotmob_etl_database_non_penalty.py)
  - match_stats.home/away_expected_goals_non_penalty for xG (FotMob computes
    this directly per shot, so no separate non-penalty match_stats table is needed)
...
OUTPUT_CSV = Path(__file__).parent / "prem_xg_history.csv"
